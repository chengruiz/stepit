...
#include <stepit/modular_policy/nn/neural_module.h>

namespace stepit {
namespace modular_policy {
NeuralModule::NeuralModule(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec)
    : Module(policy_spec, ModuleSpec(module_spec, "neural_module")) {
  ...
  displayFormattedBanner(60, kGreen, "NeuralModule {}", displayed_name);
...
bool NeuralModule::reset() {
  nn_->clearState();
  return true;
}

bool NeuralModule::update(const LowState &, ControlRequests &, FieldMap &context) {
...
void NeuralModule::parseFields(bool is_input, const FieldNameVec &node_names, const DataTypeVec &node_dtypes,
...
void NeuralModule::printNodeFields(const std::vector<std::string> &node_names,
...
NeuralActor::NeuralActor(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec)
    : NeuralModule(policy_spec, ModuleSpec(module_spec, "actor")) {}

NeuralEstimator::NeuralEstimator(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec)
    : NeuralModule(policy_spec, ModuleSpec(module_spec, "estimator")) {}

STEPIT_REGISTER_MODULE(neural, kDefPriority, Module::make<NeuralModule>);
STEPIT_REGISTER_MODULE(actor, kDefPriority, Module::make<NeuralActor>);
STEPIT_REGISTER_MODULE(estimator, kDefPriority, Module::make<NeuralEstimator>);
STEPIT_REGISTER_FIELD_SOURCE(action, kDefPriority, Module::make<NeuralActor>);
}  // namespace modular_policy
}  // namespace stepit
