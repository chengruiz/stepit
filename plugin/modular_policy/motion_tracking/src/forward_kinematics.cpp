...
#include <stepit/modular_policy/forward_kinematics.h>

namespace stepit {
namespace modular_policy {
ForwardKinematics::ForwardKinematics(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec)
...
bool ForwardKinematics::update(const LowState &low_state, ControlRequests &, FieldMap &context) {
  ...
  Quatf anchor_global_ori(context.at(anchor_global_ori_id_).view<float>());
  ...
  Vec3f anchor_global_pos = context.at(anchor_global_pos_id_).view<float>();
  for (std::size_t i{}; i < body_indices_.size(); ++i) {
