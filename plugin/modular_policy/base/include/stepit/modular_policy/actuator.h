#ifndef STEPIT_MODULAR_POLICY_ACTUATOR_H_
#define STEPIT_MODULAR_POLICY_ACTUATOR_H_
...
class Actuator : public Module,
                 public Interface<Actuator, const ModularPolicySpec & /* policy_spec */,
                                  const std::string & /* name */> {
 public:
  using Interface    = Interface<Actuator, const ModularPolicySpec &, const std::string &>;
  ...
  Actuator(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec);
...
class PositionActuator : public Actuator {
 public:
  PositionActuator(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec);
  bool reset() override;
...
class VelocityActuator : public Actuator {
 public:
  VelocityActuator(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec);
  bool reset() override;
  bool update(const LowState &low_state, ControlRequests &requests, FieldMap &context) override;
  void setLowCmd(LowCmd &cmd, cArrXf action) override;
...
class TorqueActuator : public Actuator {
 public:
  TorqueActuator(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec);
  bool reset() override;
  bool update(const LowState &low_state, ControlRequests &requests, FieldMap &context) override;
  void setLowCmd(LowCmd &cmd, cArrXf action) override;
...
class HybridActuator : public Actuator {
 public:
  HybridActuator(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec);
  bool reset() override;
  bool update(const LowState &low_state, ControlRequests &requests, FieldMap &context) override;
  void setLowCmd(LowCmd &cmd, cArrXf action) override;
...
#define STEPIT_REGISTER_ACTUATOR(name, priority, factory) \
  static ::stepit::modular_policy::Actuator::Registration _actuator_##name##_registration(#name, priority, factory)

#endif  // STEPIT_MODULAR_POLICY_ACTUATOR_H_
