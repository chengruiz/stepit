...
#include <stepit/modular_policy/modular_policy.h>

namespace stepit {
namespace modular_policy {
ModularPolicy::ModularPolicy(const RobotSpec &robot_spec, const std::string &home_dir)
  ...
  displayFormattedBanner(60, kGreen, "ModularPolicy {} ({}Hz)", spec_.policy_name, getControlFreq());
...
void ModularPolicy::addModule(Module::Ptr module, bool first) {
...
bool ModularPolicy::act(const LowState &low_state, ControlRequests &requests, LowCmd &cmd) {
  context_.clear();
  for (const auto &module : resolved_modules_) {
...
void ModularPolicy::exit() {
  context_.clear();
  for (const auto &module : resolved_modules_) module->exit();
}

bool ModularPolicy::isSatisfied(const std::set<FieldId> &requirements) const {
...
STEPIT_REGISTER_POLICY(modular, kDefPriority, Policy::make<ModularPolicy>);
}  // namespace modular_policy
}  // namespace stepit
