...
#include <stepit/modular_policy/action_source.h>

namespace stepit {
namespace modular_policy {
ActionHistory::ActionHistory(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec)
...
void ActionHistory::postStep(const FieldMap &context) { action_buf_.push_back(context.at(action_id_).view<float>()); }

ActionFilter::ActionFilter(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec)
...
bool ActionFilter::update(const LowState &low_state, ControlRequests &requests, FieldMap &context) {
  auto action = context.at(action_id_).view<float>();
