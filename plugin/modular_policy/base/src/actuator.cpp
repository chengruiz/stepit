#include <stepit/modular_policy/actuator.h>

namespace stepit {
namespace modular_policy {
Actuator::Actuator(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec)
...
PositionActuator::PositionActuator(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec)
    : Actuator(policy_spec, module_spec) {
  target_joint_pos_.setZero(policy_spec.dof);
  last_target_joint_pos_id_ = registerProvision("last_target_joint_pos", DataType::kFloat32, target_joint_pos_.size());
}
...
VelocityActuator::VelocityActuator(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec)
    : Actuator(policy_spec, module_spec) {
  target_joint_vel_         = bias_;
  last_target_joint_vel_id_ = registerProvision("last_target_joint_vel", DataType::kFloat32, target_joint_vel_.size());
}
...
TorqueActuator::TorqueActuator(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec)
    : Actuator(policy_spec, module_spec) {
  target_joint_tor_         = bias_;
  last_target_joint_tor_id_ = registerProvision("last_target_joint_tor", DataType::kFloat32, target_joint_tor_.size());
}
...
HybridActuator::HybridActuator(const ModularPolicySpec &policy_spec, const ModuleSpec &module_spec)
    : Actuator(policy_spec, module_spec) {
  if (config_["parameters"].hasValue()) {
